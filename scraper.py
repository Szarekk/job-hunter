...
def get_workplace(soup):
    # 1. Try a more specific extraction for Wrota BIP - usually in breadcrumbs or footer
    breadcrumb = soup.select_one('.breadcrumb')
    if breadcrumb:
        # Take the second to last item if it exists
        items = breadcrumb.select('li')
        if len(items) >= 2:
            return items[-2].get_text(strip=True)

    # 2. Try footer copyright
    ...
    if title_tag:
        ...
        for sep in [' - ', ' – ', ' | ', ': ']:
            if sep in title_text:
                ...
                for p in parts:
                    p = p.strip()
                    if "Biuletyn" not in p and "Nabór" not in p and len(p) > len(best_part):
                        best_part = p
...
async def scrape_wrota(session, url):
    ...
    # Stricter filter for job titles
    ...
    garbage = ['redakcja', 'instrukcja', 'mapa', 'szukaj', 'zaloguj', 'kontakt', 'deklaracja', 'statut', 'regulamin', 'podstawowe', 'prawny', 'kompetencje', 'majątek', 'kontrola', 'ogłoszenia', 'praca', 'start', 'dane', 'metryka']

    for a in links:
        link = urljoin(url, a['href'])
        title = a.get_text(strip=True)
        if not title or len(title) < 10: continue # Jobs usually have longer titles
        
        # Must have a job keyword AND not be in garbage
        title_l = title.lower()
